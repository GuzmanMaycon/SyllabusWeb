create or replace PACKAGE BODY PAC_CURSOR IS
  ...
  END LISTAR_TEMAS_X_GRUPO;
   ...
   BEGIN
      OPEN o_cursor FOR
      SELECT ASIGNATURA_APERTURADA.ID_ASIG_APERTURADA AS ID_ASIG_APERTURADA,
             ASIGNATURA.NOMBRE AS NOMBRE_ASIGNATURA,
             PLAN_DE_ESTUDIO.NOMBRE AS NOMBRE_PLAN,
             USUARIO.NOMBRE AS NOMBRE_COORDINADOR,
             USUARIO.APELLIDO AS APELLIDO_COORDINADOR,
             ESTADO_SYLLABUS.DESCRIPCION AS ESTADO_SYLLABUS
        FROM ASIGNATURA_APERTURADA
        JOIN ASIGNATURA ON ASIGNATURA.ID_ASIGNATURA = ASIGNATURA_APERTURADA.ID_ASIGNATURA
        JOIN PLAN_DE_ESTUDIO ON PLAN_DE_ESTUDIO.ID_PLAN_ESTUDIO = ASIGNATURA.ID_PLAN_ESTUDIO
        JOIN DOCENTE ON ASIGNATURA_APERTURADA.ID_COORDINADOR = DOCENTE.ID_USUARIO
        JOIN USUARIO ON USUARIO.ID_USUARIO = DOCENTE.ID_USUARIO
        JOIN SYLLABUS ON SYLLABUS.ID_ASIG_APERTURADA = ASIGNATURA_APERTURADA.ID_ASIG_APERTURADA
        JOIN ESTADO_SYLLABUS ON ESTADO_SYLLABUS.ID_ESTADO_SYLLABUS = SYLLABUS.ID_ESTADO
       WHERE ASIGNATURA_APERTURADA.ID_PERIODO = p_id_periodo;
   END RET_ASIG_APERT_X_PERIODO;
